...
def KeyGen(shard):
    try:  
        ...
        sp = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE, close_fds=True)      
        out, err = sp.communicate()    
        if out:
...
#ui string values
account_safe_str="You are RESPONSIBLE for keeping the following key pairs information safe!"
existing_shard1_key_str = "Input Existing Shard 1 Public Key"
existing_shard1_key_list_str = "Select Existing Shard 1 Public Key"
existing_shard2_key_str = "Input Existing Shard 2 Public Key"
existing_shard2_key_list_str = "Select Existing Shard 2 Public Key"
existing_shard3_key_str = "Input Existing Shard 3 Public Key"
existing_shard3_key_list_str = "Select Existing Shard 3 Public Key"
existing_shard4_key_str = "Input Existing Shard 4 Public Key"
existing_shard4_key_list_str = "Select Existing Shard 4 Public Key"
update_shard1_config_with_str= "Update Shard 1 config with coin base "
update_shard2_config_with_str= "Update Shard 2 config with coin base "
update_shard3_config_with_str= "Update Shard 3 config with coin base "
update_shard4_config_with_str= "Update Shard 4 config with coin base "
update_shard1_coinbase_with_str="Shard 1 config updated with coinbase value "
update_shard2_coinbase_with_str="Shard 2 config updated with coinbase value "
update_shard3_coinbase_with_str="Shard 3 config updated with coinbase value "
update_shard4_coinbase_with_str="Shard 4 config updated with coinbase value "
no_shard_1_public_key_selected_str="No shard 1 public key selected!"
no_shard_2_public_key_selected_str="No shard 2 public key selected!"
no_shard_3_public_key_selected_str="No shard 3 public key selected!"
no_shard_4_public_key_selected_str="No shard 3 public key selected!"
key_field_empty_str="Key field is empty!"
...
log_btn="Log"
local_btn="Local Blk"
network_btn="MainNet Blk"
config_btn="Config"
generate_key_pairs_btn="Generate Key Pairs"
shard1_key_gen_win_title_str=" Shard 1 Key Gen"
shard2_key_gen_win_title_str=" Shard 2 Key Gen"
shard3_key_gen_win_title_str=" Shard 3 Key Gen"
shard4_key_gen_win_title_str=" Shard 4 Key Gen"
shard1_existing_key_win_title_str=" Shard 1 Existing Key"
shard2_existing_key_win_title_str=" Shard 2 Existing Key"
shard3_existing_key_win_title_str=" Shard 3 Existing Key"
shard4_existing_key_win_title_str=" Shard 4 Existing Key"
...
menu_def = [['&File', ['E&xit']],
           ['Keys', ['Shard 1',['New::_key_s1_','Existing::_key_s1_',], 'Shard 2',['New::_key_s2_','Existing::_key_s2_',], 'Shard 3',['New::_key_s3_','Existing::_key_s3_',], 'Shard 4',['New::_key_s4_','Existing::_key_s4_',]]],
           ['Help', ['Mining::_help_mining_', 'About::_help_about_']],
           ]
...
layout = [
            [sg.Menu(menu_def, )],
            [sg.TabGroup([[sg.Tab('Shard 1', shard_1_layout, key='_shard1_'), sg.Tab('Shard 2', shard_2_layout, key='_shard2_'),sg.Tab('Shard 3', shard_3_layout, key='_shard3_'),sg.Tab('Shard 4', shard_4_layout, key='_shard4_')]])],
            [sg.T('')]        
         ]

window = sg.Window(' Seele Mining Control', layout, icon='icon/seele.ico', resizable=True,  font=("Arial", 10), element_padding=((5,5),(5,5))).Finalize() 
...
while True:
    ...
    if not shard1_key_window_active and event == 'New::_key_s1_':
    ...
    if not shard2_key_window_active and event == 'New::_key_s2_':
    ...
    if not shard3_key_window_active and event == 'New::_key_s3_':
    ...
    if not shard4_key_window_active and event == 'New::_key_s4_':
    ...
    if not shard1_key_exist_window_active and event == 'Existing::_key_s1_':
    ...
    if not shard2_key_exist_window_active and event == 'Existing::_key_s2_':
    ...
    if not shard3_key_exist_window_active and event == 'Existing::_key_s3_':
    ...
    if not shard4_key_exist_window_active and event == 'Existing::_key_s4_':
    ...
    if shard4_key_exist_window_active:
        while True:
            ...
            if event_key_exist_s4 is None or event_key_exist_s4 == 'Exit':
                ...
                break
    #help window
    if not help_window_active and event == 'Mining::_help_mining_':
        ...
        win_help = sg.Window("Help", icon='icon/seele.ico', resizable=True, font=("Arial", 10), element_padding=((5,5),(5,5))).Layout(layout_help)

        if help_window_active:
            while True:
                event_help, value_help = win_help.Read(timeout=100)
                if event_help == "Mining::_help_mining_":
                    pass
                ...
                if event_help is None or event_help == 'Exit':
                    ...
                    break
    #about window
    if not about_window_active and event == 'About::_help_about_':
        ...
        if about_window_active:
            while True:
                ...
                if event_about == "About::_help_about_":
                    pass
